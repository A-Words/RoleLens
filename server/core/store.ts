...
export class Store {
  ...
  constructor(public directory: string) {
    ...
    `)
    const columns = this.db.prepare('PRAGMA table_info(generations)').all() as { name: string }[]
    if (!columns.some((c) => c.name === 'evidence'))
      this.db.exec("ALTER TABLE generations ADD COLUMN evidence TEXT NOT NULL DEFAULT '[]'")
  }
  revision() {
    return (
      this.db.prepare("SELECT value FROM meta WHERE key='revision'").get() as { value: number }
    ).value
  }
  bump() {
    this.db.prepare("UPDATE meta SET value=value+1 WHERE key='revision'").run()
  }
  ...
  facts(): Fact[] {
    return (this.db.prepare('SELECT * FROM facts ORDER BY updatedAt DESC').all() as Fact[]).map(
  ...
  eligible() {
    return this.facts().filter((f) => f.enabled && f.category !== 'contact')
  }
  search(query: string): Fact[] {
    const terms = query.toLowerCase().match(/[a-z0-9+#.]+|[\u4e00-\u9fff]{2,}/g) || []
    const tokens = terms.flatMap((t) =>
      /[\u4e00-\u9fff]/.test(t)
        ? [t, ...Array.from({ length: Math.max(0, t.length - 1) }, (_, i) => t.slice(i, i + 2))]
        : [t],
    )
    return this.eligible()
      .map((f) => ({
        f,
        score: tokens.reduce(
          (n, t) => n + (`${f.title} ${f.content}`.toLowerCase().includes(t) ? 1 : 0),
          0,
        ),
      }))
  ...
  draft(input: FactInput, sourceId: string, targetId: string | null = null): Draft {
    ...
    this.db
      .prepare(
        'INSERT INTO drafts VALUES (@id,@category,@title,@content,@enabled,@sourceId,@targetId,@expectedVersion,@status)',
      )
      .run({ ...d, enabled: +d.enabled })
    return d
  }
  drafts(): Draft[] {
    return (this.db.prepare("SELECT * FROM drafts WHERE status='pending'").all() as Draft[]).map(
  ...
  confirm(id: string, input: FactInput) {
    ...
    return this.db.transaction(() => {
      ...
      if (d.targetId) f = this.update(d.targetId, input, d.expectedVersion!, d.sourceId)
        ...
        this.db
          .prepare(
            'INSERT INTO facts VALUES (@id,@category,@title,@content,@enabled,@sourceId,@version,@updatedAt)',
          )
          .run({ ...f, enabled: +f.enabled })
        this.snapshot(f)
        this.bump()
      }
  ...
  discard(id: string) {
    this.db.prepare("UPDATE drafts SET status='discarded' WHERE id=? AND status='pending'").run(id)
  }
  snapshot(f: Fact) {
    this.db
      .prepare('INSERT INTO fact_versions VALUES (?,?,?)')
      .run(f.id, f.version, JSON.stringify(f))
  }
  versions(id: string) {
    return (
      this.db
        .prepare('SELECT snapshot FROM fact_versions WHERE id=? ORDER BY version DESC')
        .all(id) as { snapshot: string }[]
    ).map((r) => JSON.parse(r.snapshot) as Fact)
  }
  update(id: string, input: FactInput, version: number, sourceId?: string): Fact {
    ...
    return this.db.transaction(() => {
      ...
      this.db
        .prepare(
          'UPDATE facts SET category=@category,title=@title,content=@content,enabled=@enabled,sourceId=@sourceId,version=@version,updatedAt=@updatedAt WHERE id=@id',
        )
        .run({ ...f, enabled: +f.enabled })
      this.snapshot(f)
      this.bump()
  ...
  createJob(input: Omit<Job, 'id' | 'createdAt'>) {
    const j = { ...input, id: uid(), createdAt: now() }
    this.db.prepare('INSERT INTO jobs VALUES (@id,@company,@title,@jd,@createdAt)').run(j)
    return j
  }
  createSession(jobId: string) {
    ...
    this.db
      .prepare(
        'INSERT INTO sessions VALUES (@id,@jobId,@status,@revision,@analysis,@question,@error,@createdAt)',
      )
      .run(s)
    return this.session(s.id)
  }
  session(id: string): Session {
    const s = this.db.prepare('SELECT * FROM sessions WHERE id=?').get(id) as
      | (Omit<Session, 'analysis' | 'question'> & { analysis: string | null; question: string })
      | undefined
  ...
  setSession(
    ...
    this.db
      .prepare('UPDATE sessions SET status=?,analysis=?,question=?,error=? WHERE id=?')
      .run(
        status,
        JSON.stringify(analysis ?? old.analysis),
        JSON.stringify(question ?? old.question),
        error,
        id,
      )
  }
  trace(sessionId: string, event: string, detail: unknown) {
    this.db
      .prepare('INSERT INTO traces (sessionId,event,detail,createdAt) VALUES (?,?,?,?)')
      .run(sessionId, event, JSON.stringify(detail), now())
  }
  traces(sessionId: string) {
    return this.db
      .prepare('SELECT * FROM traces WHERE sessionId=? ORDER BY id')
      .all(sessionId) as Trace[]
  }
  save(sessionId: string, content: Resume): Generation {
    const s = this.session(sessionId)
    const ids = new Set(
      [...content.sections.flatMap((s) => s.items), ...content.greetings].flatMap((i) => i.factIds),
    )
    const g = {
      id: uid(),
      jobId: s.jobId,
      ...
      evidence: JSON.stringify(this.eligible().filter((f) => ids.has(f.id))),
      createdAt: now(),
      edited: 0,
      version: 1,
    }
    this.db
      .prepare(
        'INSERT OR IGNORE INTO generations (id,jobId,sessionId,content,createdAt,edited,version,evidence) VALUES (@id,@jobId,@sessionId,@content,@createdAt,@edited,@version,@evidence)',
      )
      .run(g)
    return this.generations(s.jobId).find((g) => g.sessionId === sessionId)!
  }
  generations(jobId: string): Generation[] {
    return (
      this.db
        .prepare('SELECT * FROM generations WHERE jobId=? ORDER BY createdAt DESC')
        .all(jobId) as (Omit<Generation, 'content' | 'evidence'> & {
        content: string
        evidence: string
      })[]
    ).map((g) => ({
  ...
  generation(id: string): Generation {
    const g = this.db.prepare('SELECT * FROM generations WHERE id=?').get(id) as
      (Omit<Generation, 'content' | 'evidence'> & { content: string; evidence: string }) | undefined
  ...
  editGeneration(id: string, content: Resume, version: number) {
    const result = this.db
      .prepare(
        'UPDATE generations SET content=?,edited=1,version=version+1 WHERE id=? AND version=?',
      )
      .run(JSON.stringify(content), id, version)
...
export function getStore() {
  return (singleton ??= new Store(resolve(process.env.ROLELENS_DATA_DIR || '.data')))
}
