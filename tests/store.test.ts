...
function setup() { const dir = mkdtempSync(join(tmpdir(), 'rolelens-')); dirs.push(dir); const s = new Store(dir); stores.push(s); return s }
const input = { category: 'project' as const, title: '前端项目', content: '使用 Vue 和 TypeScript 实现网页', enabled: true }
function fact(s: Store) { return s.confirm(s.draft(input, s.source('测试', input.content).id).id, input) }
afterEach(() => { stores.forEach(s => { if (s.db.open) s.close() }); dirs.forEach(d => rmSync(d, { recursive: true, force: true })); stores.length = 0; dirs.length = 0 })
test('drafts require confirmation; confirmation is atomic and rejects duplicate', () => {
  const s = setup(), d = s.draft(input, s.source('资料', input.content).id)
...
test('stale edits and stale draft confirmation cannot overwrite newer facts', () => {
  const s = setup(), f = fact(s), d = s.draft(input, f.sourceId, f.id)
...
test('excluded, contact and deleted facts never enter retrieval; changes persist', () => {
  ...
  expect(s.eligible()).toHaveLength(0)
  s.deleteFact(f.id, 2)
  const directory = s.directory; s.close()
