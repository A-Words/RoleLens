...
describe('local runtime configuration', () => {
  ...
  it('resolves overrides and reports only field names', () => {
    ...
    expect(provider.view().effectiveSettings.model).toBe('override')
    expect(provider.view().effectiveSettings).not.toHaveProperty('apiKey')
    expect(provider.view().effectiveSettings).not.toHaveProperty('langfuseSecretKey')
